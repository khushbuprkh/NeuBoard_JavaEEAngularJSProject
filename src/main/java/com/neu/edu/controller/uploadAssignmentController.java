...
@Controller
@RequestMapping("/uploadAssignment.htm")
public class uploadAssignmentController {
	...
	public String handleFileUpload(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			Courses course = cd.get(assignment.getCourseName());
			int posspoints = assignment.getPossiblepoints();

			Assignment a=assignmentDao.create(assignment.getWriteQuestion(), posspoints, course);
